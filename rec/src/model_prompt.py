...
class KGPrompt(nn.Module):
    def __init__(
        ...
        self.entity_proj_shared = nn.Linear(hidden_size, hidden_size)
        self.token_proj_shared = nn.Linear(hidden_size, hidden_size)
       
        # self.entity_proj1_shared = nn.Sequential(
        #     nn.Linear(hidden_size, hidden_size // 2),
        #     nn.ReLU(),
        #     nn.Linear(hidden_size // 2, hidden_size),
        # )
    
        self.cross_attn_shared = nn.Linear(hidden_size, hidden_size, bias=False)
        
    
        # self.token_proj1_shared = nn.Sequential(
        #     nn.Linear(hidden_size, hidden_size // 2),
        #     nn.ReLU(),
        #     nn.Linear(hidden_size // 2, hidden_size),
        # )
        self.Wgate = nn.Linear(2 * self.hidden_size, 1)  
    ...
    def get_adust_item_embeds(self, entity_embeds=None, token_embeds=None, fusion_mode: str = 'cross_attn'):

        attn_weights = self.cross_attn_shared(token_embeds) @ entity_embeds.T  
        attn_weights /= self.hidden_size  
        entity_weights = F.softmax(attn_weights, dim=1) 
