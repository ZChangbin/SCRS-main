...
from utils import init_wandb_run, GENERATION, PROJECT_NAME, MODEL_NAME, wandb_logging, freeze_model_params, count_parameters, save, load, save_gen_model, load_gen_model
...
def parse_args():
    ...
    parser.add_argument('--num_warmup_steps', type=int, default=967)
...
if __name__ == '__main__':
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (train_batch) in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                trian_token_embeds = text_encoder(**train_batch['prompt']).last_hidden_state
            trian_prompt_augmented_input_embeddings, trian_new_attention_mask, _, _ = prompt_encoder(
                entity_ids=train_batch['entity'],
                token_embeds=trian_token_embeds,
                output_entity=False,
                use_conv_prefix=True,
                mapping = args.mapping,
                word_embeddings = model.pretrained_model.get_input_embeddings().weight,
                context_input_embeddings = model.pretrained_model.get_input_embeddings()(train_batch['context']['input_ids']),
                attention_mask = train_batch['context']['attention_mask']
            )
            
            train_batch['context']['input_ids'] = None

            train_batch['context']['inputs_embeds'] = trian_prompt_augmented_input_embeddings
            train_batch['context']['attention_mask'] = trian_new_attention_mask
            
            pad_resp = -100 * torch.ones((trian_new_attention_mask.shape[0], args.n_examples * args.prompt_max_length)).to(trian_new_attention_mask.device).long()
            train_batch['resp'] = torch.cat([pad_resp, train_batch['resp']], dim =1)

            loss = model.pretrained_model(
                 inputs_embeds = train_batch['context']['inputs_embeds'],
                 input_ids = None,
                 labels = train_batch['resp'], 
                 return_dict = True
        ...
        for batch in tqdm(valid_dataloader, disable=not accelerator.is_local_main_process):
            ...
            with torch.no_grad():
                ...
                prompt_augmented_input_embeddings, new_attention_mask, _, _ = prompt_encoder(
                    ...
                    mapping = args.mapping,
                    word_embeddings = model.pretrained_model.get_input_embeddings().weight,
                    context_input_embeddings = model.pretrained_model.get_input_embeddings()(batch['context']['input_ids']),
                ...
                loss = model.pretrained_model(inputs_embeds = batch['context']['inputs_embeds'],
                    input_ids = None,
                    labels = batch['resp'], 
                    return_dict = True
                )['loss']               
