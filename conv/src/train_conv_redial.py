...
def parse_args():
    ...
    parser.add_argument("--num_warmup_steps", type=int, default=3173)
...
if __name__ == "__main__":
    ...
    model, prompt_encoder, optimizer, train_dataloader = accelerator.prepare(
        model, prompt_encoder, optimizer, train_dataloader
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, train_batch in enumerate(train_dataloader):
            ...
            pad_resp = -100 * torch.ones(
                (trian_new_attention_mask.shape[0], args.n_examples * args.prompt_max_length)
            ).to(trian_new_attention_mask.device).long()
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                ...
                completed_steps += 1
                if run:
                    run.log({"loss": sum(train_loss) / len(train_loss) * args.gradient_accumulation_steps})
            if completed_steps >= args.max_train_steps:
                break

        train_loss = sum(train_loss) / len(train_loss) * args.gradient_accumulation_steps
