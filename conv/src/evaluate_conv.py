import re
import json
from nltk import ngrams
from nltk.translate.bleu_score import sentence_bleu
from rouge import Rouge
import numpy as np

slot_pattern = re.compile(r'<movie>')

def _cal_rouge(hypothesis, reference):
    if hypothesis == '':
        return 0,0,0
    rouge = Rouge()
    try:
        scores = rouge.get_scores(hypothesis, reference)
        return scores[0]['rouge-1']['f'],scores[0]['rouge-2']['f'],scores[0]['rouge-l']['f']
    except:
        print("something wrong here! ",hypothesis,reference)
        return 0,0,0




def _compute_rouge_L(tokenizer, preds, labels, num_i):
    decoded_preds = tokenizer.batch_decode(preds, skip_special_tokens=False)
    decoded_labels = tokenizer.batch_decode(labels, skip_special_tokens=False)

    decoded_preds = ['System: ' + pred.replace('<pad>', '').replace('<|endoftext|>', '').strip() 
                    for pred in decoded_preds]
    decoded_labels = [label.replace('<pad>', '').replace('<|endoftext|>', '').strip() 
                      for label in decoded_labels]

    """
    both hypothesis and reference are str
    """
    rouge = Rouge()
    rouge_scores = []

    expanded_labels = np.repeat(decoded_labels, num_i)

    for pred, label in zip(decoded_preds, expanded_labels):
        if not pred.strip(): 
            rouge_scores.append(0)
            continue
        try:
            scores = rouge.get_scores(pred, label)
            rouge_scores.append(scores[0]['rouge-l']['f'])
        except Exception as e:
            print("Error computing ROUGE-L:", e, "Pred:", pred, "Label:", label)
            rouge_scores.append(0)

    
    return np.array(rouge_scores).reshape(-1, num_i)
    


def _compute_distk(tokenizer, preds, num_i, k):
    decoded_preds = tokenizer.batch_decode(preds, skip_special_tokens=False)
    decoded_preds = ['System: ' + pred.replace('<pad>', '').replace('<|endoftext|>', '').strip() 
                     for pred in decoded_preds]

    distk_scores = []

    for pred in decoded_preds:
        tokens = pred.split()  
        if len(tokens) < k:  
            distk_scores.append(0)
            continue
        
        k_grams = list(ngrams(tokens, k))  
        unique_four_grams = set(k_grams)  
        distk = len(unique_four_grams) / len(k_grams) if k_grams else 0
        distk_scores.append(distk)

    return np.array(distk_scores).reshape(-1, num_i)



class ConvEvaluator:
    def __init__(self, tokenizer, log_file_path):
        self.tokenizer = tokenizer

        self.reset_metric()
        if log_file_path:
            self.log_file = open(log_file_path, 'w', buffering=1)
            self.log_cnt = 0

    def evaluate(self, preds, labels, log=False):

        decoded_preds = self.tokenizer.batch_decode(preds, skip_special_tokens=False)
        decoded_preds = [decoded_pred.replace('<pad>', '').replace('<|endoftext|>', '') for decoded_pred in
                         decoded_preds]
        
        decoded_preds = [pred.strip() for pred in decoded_preds]
        decoded_preds = ['System: ' + pred for pred in decoded_preds]
        
        decoded_labels = self.tokenizer.batch_decode(labels, skip_special_tokens=False)
        decoded_labels = [decoded_label.replace('<pad>', '').replace('<|endoftext|>', '') for decoded_label in
                          decoded_labels]
        decoded_labels = [label.strip() for label in decoded_labels]
        if log and hasattr(self, 'log_file'):
            for pred, label in zip(decoded_preds, decoded_labels):
                self.log_file.write(json.dumps({
                    'pred': pred,
                    'label': label
                }, ensure_ascii=False) + '\n')

        self.collect_ngram(decoded_preds)
        self.compute_item_ratio(decoded_preds)
        self.compute_bleu(decoded_preds, decoded_labels)
        self.compute_rouge(decoded_preds, decoded_labels)
        self.sent_cnt += len([pred for pred in decoded_preds if len(pred) > 0])

    def collect_ngram(self, strs):
        for str in strs:
            str = str.split()
            for k in range(1, 5):
                dist_k = f'dist@{k}'
                for token in ngrams(str, k):
                    self.metric[dist_k].add(token)

    def compute_bleu(self, preds, labels):
        for pred, label in zip(preds, labels):
            pred, label = pred.split(), [label.split()]
            for k in range(4):
                weights = [0] * 4
                weights[k] = 1
                self.metric[f'bleu@{k + 1}'] += sentence_bleu(label, pred, weights)
        

    def compute_rouge(self, preds, labels):
        
        for pred, label in zip(preds, labels):
            rouge_1, rouge_2, rouge_l = _cal_rouge(pred, label)
            self.metric["rouge@1"] += rouge_1
            self.metric["rouge@2"] += rouge_2
            self.metric["rouge@L"] += rouge_l

    def compute_item_ratio(self, strs):
        for str in strs:
            items = re.findall(slot_pattern, str)
            self.metric['item_ratio'] += len(items)

    def report(self):
        report = {}
        for k, v in self.metric.items():
            if self.sent_cnt == 0:
                report[k] = 0
            else:
                if 'dist' in k:
                    v = len(v)
                report[k] = v / self.sent_cnt
        report['sent_cnt'] = self.sent_cnt
        return report

    def reset_metric(self):
        self.metric = {
            'bleu@1': 0,
            'bleu@2': 0,
            'bleu@3': 0,
            'bleu@4': 0,
            'rouge@1': 0,
            'rouge@2': 0,
            'rouge@L': 0,
            'dist@1': set(),
            'dist@2': set(),
            'dist@3': set(),
            'dist@4': set(),
            'item_ratio': 0,
        }
        self.sent_cnt = 0
